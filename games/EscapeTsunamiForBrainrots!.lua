...
M.S = {
    Farming = false,
    FarmTargets = {"Brainrots"}, -- NEW: Master Farm Target (Brainrots, Lucky Blocks)
    SelectedBrainrots = {},
    TargetMutation = "None",
    TargetRarity = {"Common"},
...
-- FIXED: TweenTo function agar tidak error kalkulasinya
...
function M.undergroundPathTo(targetCFrame)
    ...
    if not bv then
        bv = Instance.new("BodyVelocity") bv.Name = "AntiFallMzD" bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
        bv.Velocity = Vector3.new(0, 0, 0) bv.Parent = hrp
    end
    local startPos = hrp.Position local endPos = targetCFrame.Position local DEEP_Y = -25
    M.fastTween(CFrame.new(startPos.X, DEEP_Y, startPos.Z)) task.wait(0.05)
    M.tweenTo(CFrame.new(endPos.X, DEEP_Y, endPos.Z)) task.wait(0.05)
    M.tweenTo(targetCFrame) task.wait(0.05) return true
end

function M.undergroundReturnToBase()
    local ch = Player.Character if not ch then return false end
    local hrp = ch:FindFirstChild("HumanoidRootPart") if not hrp then return false end
    local curPos = hrp.Position local homePos = M.getHomePosition().Position local DEEP_Y = -25
    local bv = hrp:FindFirstChild("AntiFallMzD")
    if not bv then bv = Instance.new("BodyVelocity") bv.Name = "AntiFallMzD" bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge) bv.Velocity = Vector3.new(0, 0, 0) bv.Parent = hrp end
    M.fastTween(CFrame.new(curPos.X, DEEP_Y, curPos.Z)) task.wait(0.05)
    M.tweenTo(CFrame.new(homePos.X, DEEP_Y, homePos.Z)) task.wait(0.05)
    M.tweenTo(M.getHomePosition()) task.wait(0.05)
    if bv then bv:Destroy() end return true
end
...
function M.forceGrabPrompt(target)
    ...
    for _, p in pairs(prompts) do
        ...
        pcall(function() fireproximityprompt(p) end) pcall(function() fireproximityprompt(p) end)
    end
    
    local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
    if hrp then
        local parent = target if parent:IsA("ProximityPrompt") then parent = parent.Parent end
        if parent and parent:IsA("BasePart") then pcall(function() firetouchinterest(hrp, parent, 0) firetouchinterest(hrp, parent, 1) end) end
        local searchRoot = parent if searchRoot and searchRoot.Parent and not searchRoot.Parent:IsA("Workspace") then searchRoot = searchRoot.Parent end
        if searchRoot then for _, d in pairs(searchRoot:GetDescendants()) do if d:IsA("BasePart") then pcall(function() firetouchinterest(hrp, d, 0) firetouchinterest(hrp, d, 1) end) end end end
    end task.wait(0.04)
end
...
function M.matchesFilter(b, folderRarity)
    ...
    if M.S.TargetMutation == "None" then if not isNone then return false end elseif M.S.TargetMutation ~= "Any" then if mut ~= M.S.TargetMutation then return false end end
    return true
end
...
function M.toolMatchesRarity(tool, targetRarity, targetMutation)
    ...
    if not isAny then
        if toolRarity and toolRarity ~= "" then
            local m = false for _, r in pairs(tR) do if toolRarity == r then m = true break end end if not m then return false end
        else
...
function M.tweenToSlot(slotNumber)
    ...
    local myBase = workspace:FindFirstChild("Bases") and workspace.Bases:FindFirstChild(M.baseGUID) if not myBase then return false end
    local sm = myBase:FindFirstChild("slot " .. slotNumber .. " brainrot") if not sm then return false end
    local root = sm:FindFirstChild("Root") if root then return M.tweenTo(root.CFrame * CFrame.new(0, 3, 0)) end
    for _, part in pairs(sm:GetDescendants()) do if part:IsA("BasePart") then return M.tweenTo(part.CFrame * CFrame.new(0, 3, 0)) end end return false
end

function M.isHighRarityTool(tool)
    if not tool then return false end local r = tool:GetAttribute("Rarity") or "" if HIGH_RARITIES[r] then return true end
    ...
    if M.ActiveBrainrots then
        for _, folder in pairs(M.ActiveBrainrots:GetChildren()) do
            if HIGH_RARITIES[folder.Name] then for _, b in pairs(folder:GetChildren()) do if (b:GetAttribute("BrainrotName") or "") == bName then return true end end end
        end
    end return false
end
...
function M.luckyBlockMutationMatches(block)
    ...
    if M.S.LuckyBlockMutation == "Any" then return true end if M.S.LuckyBlockMutation == "None" then return isNone end
    return mut == M.S.LuckyBlockMutation
end
function M.luckyBlockGetRarityFromName(bn) return bn:match("LuckyBlock_(.+)") or bn end
...
function M.hasFarmTarget(targetName)
    for _, v in pairs(M.S.FarmTargets) do if v == targetName then return true end end
    return false
end

function M.startFarming()
    if M.farmThread then return end
    M.S.Farming = true M.Status.farmCount = 0 M.Status.luckyBlockCount = 0
    M.setHomePosition() M.detectWallZ() M.returnToBase() M.enableNoclip() M.enableGod()
        ...
        while M.S.Farming do
            local ok, err = pcall(function()
                if M.isDead() then
                    ...
                    M.waitForRespawn() task.wait(1) M.setHomePosition() M.enableGod() task.wait(0.5) 
                    currentBackpackCount = 0 return
                end
                local ch = Player.Character local hum = ch and ch:FindFirstChild("Humanoid")
                if not ch or not hum then task.wait(1) return end
                
                -- === 1. PRIORITAS: LUCKY BLOCKS ===
                        ...
                        for _, block in pairs(M.ActiveLuckyBlocks:GetChildren()) do
                            ...
                            if M.luckyBlockRarityMatches(block.Name) and M.luckyBlockMutationMatches(block) then
                                ...
                                for attempt = 1, 5 do
                                    ...
                                    if M.isDead() then M.waitForRespawn() task.wait(1) M.setHomePosition() M.enableGod()
                                        if rootPart and rootPart.Parent then M.undergroundPathTo(rootPart.CFrame * CFrame.new(0, 3, 0)) else break end
                                    end
                                    if rootPart and rootPart.Parent then 
                                        M.forceGrabPrompt(block) task.wait(0.04) 
                                        M.forceGrabPrompt(rootPart) task.wait(0.04)
                                        if not block.Parent or not rootPart.Parent then M.Status.luckyBlockCount += 1 end 
                                        break
                                    else break end
                                end
                                pcall(function() hum:UnequipTools() end) task.wait(0.04)
                                
                                local hrp = ch:FindFirstChild("HumanoidRootPart")
                                if hrp then M.fastTween(CFrame.new(hrp.Position.X, -25, hrp.Position.Z)) end
                ...
                -- === 2. PRIORITAS KEDUA: BRAINROTS ===
                if M.hasFarmTarget("Brainrots") then
                    if not M.baseGUID then M.findBase() end
                    if not M.baseGUID then M.Status.farm = "No base found!" task.wait(2) return end
                                    ...
                                    for _, b in pairs(folder:GetChildren()) do
                                        ...
                                        if M.matchesFilter(b, folder.Name) then
                                            ...
                                            for attempt = 1, 5 do
                                                if not M.S.Farming then break end
                                                if M.isDead() then M.waitForRespawn() task.wait(1) M.setHomePosition() M.enableGod()
                                                    if root and root.Parent then M.undergroundPathTo(root.CFrame * CFrame.new(0, 3, 0)) else break end
                                                end
                                                if root and root.Parent then M.Status.farm = "Grabbing " .. folder.Name .. "..."
                                                    M.forceGrabPrompt(root) M.forceGrabPrompt(b) task.wait(0.04) 
                                                    M.Status.farmCount += 1 currentBackpackCount += 1 break
                                                else break end
                                            end
                                            pcall(function() hum:UnequipTools() end) task.wait(0.04)
                                            local hrp = ch:FindFirstChild("HumanoidRootPart")
                                            ...
                                            if currentBackpackCount >= maxBackpackCount then
                                                ...
                                                M.undergroundReturnToBase() currentBackpackCount = 0
                                            end
                                            return -- Restart loop
                                        end
                                    end
                    ...
                    if not tool then
                            ...
                            for _, folder in pairs(M.ActiveBrainrots:GetChildren()) do
                                ...
                                if folder:IsA("Folder") and M.rarityMatches(folder.Name) then
                                    for _, b in pairs(folder:GetChildren()) do
                                        ...
                                        if M.matchesFilter(b, folder.Name) then
                                            local root = M.findBrainrotRoot(b) if not root then continue end
                                            found = true M.Status.farm = "Retrieving " .. folder.Name .. "..."
                                            M.undergroundPathTo(root.CFrame * CFrame.new(0, 3, 0))
                                            
                                            for attempt = 1, 5 do
                                                if not M.S.Farming then break end
                                                if M.isDead() then M.waitForRespawn() task.wait(1) M.setHomePosition() M.enableGod()
                                                    if root and root.Parent then M.undergroundPathTo(root.CFrame * CFrame.new(0, 3, 0)) else found = false break end
                                                end
                                                if root and root.Parent then M.forceGrabPrompt(root) M.forceGrabPrompt(b)
                                                    task.wait(0.04) M.Status.farmCount += 1 break
                                            ...
                                            M.Status.farm = "Returning underground..." M.undergroundReturnToBase() 
                                            break
                                        end
                                    end
                                end
                        ...
                        if not found then M.Status.farm = "Searching Targets..." task.wait(2) return end
                        task.wait(0.3) tool = M.findTargetToolInBackpack()
                        if not tool then return end
                    end

                    if M.isHighRarityTool(tool) then M.Status.farmCount += 1 task.wait(0.5) return end
                    ...
                    if M.isSlotEmpty(ws) then M.Status.farm = "Placement failed..." pcall(function() hum:UnequipTools() end) task.wait(1) return end
                    ...
                    M.Status.farm = bName .. " DONE!" task.wait(0.3)
            ...
            if not ok then warn("[Catraz Farm] " .. tostring(err)) task.wait(1) end
            task.wait(0.3)
        end
...
function M.stopFarming()
    ...
    pcall(function()
        local ch = Player.Character if ch then
            local hrp = ch:FindFirstChild("HumanoidRootPart")
            if hrp then local bv = hrp:FindFirstChild("AntiFallMzD") if bv then bv:Destroy() end end
        end
    end)
    M.Status.farm = "Idle"
end

-- EVENT FARMS, FACTORY, ETC. (Tetap sama)
    ...
    M.factoryThread = task.spawn(function()
        ...
        while M.S.FactoryEnabled do
            local ok = pcall(function()
                ...
                if not M.baseGUID then M.findBase() end if not M.baseGUID then M.Status.factory = "No base found!" task.wait(2) return end
                ...
                local bp = Player:FindFirstChild("Backpack") if not bp then return end local tool = nil
                for _, t in pairs(bp:GetChildren()) do if t:IsA("Tool") and M.toolMatchesRarity(t, M.S.FactoryRarity, "None") then tool = t break end end
                if not tool then stopR = "All " .. M.S.FactoryRarity .. "s maxed!" M.S.FactoryEnabled = false return end
                local bName = tool:GetAttribute("BrainrotName") or "Item" M.Status.factory = "Equipping " .. bName
                local hum = Player.Character and Player.Character:FindFirstChild("Humanoid") if hum then hum:EquipTool(tool) task.wait(0.5) end
                M.placeBrainrot(ws) task.wait(0.8)
            ...
            if not ok then task.wait(1) end if M.S.FactoryEnabled then task.wait(0.5) end
        end
...
function M.stopFactoryLoop()
    ...
    if not (string.find(M.Status.factory or "", "maxed") or string.find(M.Status.factory or "", "money")) then M.Status.factory = "Idle" end
end

function M.startMoney()
    if M.moneyThread then return end M.S.AutoCollectMoney = true M.Status.money = "Active"
    if not M.baseGUID then M.findBase() end
    M.moneyThread = task.spawn(function()
        while M.S.AutoCollectMoney do pcall(function()
                if not M.baseGUID then M.findBase() end if not M.baseGUID then return end
                local mb = workspace:FindFirstChild("Bases") and workspace.Bases:FindFirstChild(M.baseGUID)
                local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") if not mb or not hrp then return end
                for i = 1, 40 do
                    local sm = mb:FindFirstChild("slot " .. i .. " brainrot")
            ...
            end) task.wait(0.1) end M.Status.money = "Idle"
    end)
...
function M.startAutoUpgrade()
    if M.upgradeThread then return end M.S.AutoUpgrade = true M.Status.upgradeCount = 0
    M.upgradeThread = task.spawn(function()
        while M.S.AutoUpgrade do pcall(function()
            ...
            end) task.wait(3) end M.Status.upgrade = "Idle"
    end)
end
...
local WindowConfig = {
    Name = "Catraz Hub", Subtext = "Escape Tsunami For Brainrots", Version = "v1.1.0 - PRO",
    VersionIcon = "crown", TagColor = Color3.fromRGB(200, 40, 40), ShowIcon = true,
    Icon = "rbxassetid://105921924721005", ImageBackground = "rbxassetid://84894412677021",
    ImageTransparency = 0.1, WindowTransparency = 0.1, SaveConfig = true,
    ConfigFolder = "CatrazHub_Data", IntroEnabled = true, IntroText = "Welcome to Catraz Hub, " .. LPlayer.DisplayName,
    IntroIcon = "rbxassetid://105921924721005", ToggleIcon = "rbxassetid://105921924721005", ToggleSize = 50
}
...
local HomeTab = Window:MakeTab({ Name = "Home", Icon = "home", Glass = true, Outline = false })
local DashSec = HomeTab:AddSection({ Name = "Dashboard" })
DashSec:AddParagraph({ Title = "Welcome, " .. LPlayer.DisplayName, Desc = "Catraz Hub v1.1.0 - PRO | Escape Tsunami For Brainrots", Image = "rbxthumb://type=AvatarHeadShot&id=" .. LPlayer.UserId .. "&w=150&h=150", ImageSize = 45 })

-- ================== TAB 2: UNIFIED FARM ==================
local FarmTab = Window:MakeTab({ Name = "Farm", Icon = "swords", Glass = false, Outline = true })

local FarmFilterSec = FarmTab:AddSection({ Name = "🎯 Target Selection", Glass = false, Outline = true })

-- DROPDOWN TARGET MASTER
FarmFilterSec:AddDropdown({
    Name = "What to Farm?", Default = {"Brainrots"}, Options = {"Brainrots", "Lucky Blocks"},
    Multi = true, Outline = true, Flag = "FarmTargets",
    Callback = function(v)
        local s={} for _, on in pairs(v) do table.insert(s, on) end
        if #s==0 then s={"Brainrots"} end
        M.S.FarmTargets = s 
    end
})

local BrainrotSet = FarmTab:AddSection({ Name = "🧟 Brainrot Settings", Glass = false, Outline = true })
BrainrotSet:AddDropdown({ Name = "Target Rarity", Default = "Common", Options = RAR, Multi = true, Outline = true, Callback = function(v) local s={} for _, on in pairs(v) do table.insert(s, on) end M.S.TargetRarity = #s>0 and s or {"Common"} end })
BrainrotSet:AddDropdown({ Name = "Target Mutation", Default = "None", Options = MUT, Multi = false, Outline = true, Callback = function(v) M.S.TargetMutation = v end })
BrainrotSet:AddDropdown({ Name = "Farm Mode", Default = M.S.FarmMode, Options = FM, Multi = false, Outline = true, Callback = function(v) M.S.FarmMode = v end })
BrainrotSet:AddDropdown({ Name = "Work Slot", Default = M.S.FarmSlot, Options = SL, Multi = false, Outline = true, Callback = function(v) M.S.FarmSlot = v end })
BrainrotSet:AddSlider({ Name = "Max Level", Min = 1, Max = 500, Default = M.S.MaxLevel, Increment = 1, ValueName = "Lv", Outline = false, Callback = function(v) M.S.MaxLevel = math.floor(v) end })

local LBSet = FarmTab:AddSection({ Name = "🎲 Lucky Block Settings", Glass = false, Outline = true })
LBSet:AddDropdown({ Name = "LB Rarity", Default = "Common", Options = LBR, Multi = true, Outline = true, Callback = function(v) local s={} for _, on in pairs(v) do table.insert(s, on) end M.S.LuckyBlockRarity = #s>0 and s or {"Common"} end })
LBSet:AddDropdown({ Name = "LB Mutation", Default = "Any", Options = MUT, Multi = false, Outline = true, Callback = function(v) M.S.LuckyBlockMutation = v end })

local FarmControlSec = FarmTab:AddSection({ Name = "🚀 Auto Farm Master", Glass = false, Outline = true })
local FSP = FarmControlSec:AddParagraph({Title = "Master Farm Status", Desc = "Idle"})
local FPP = FarmControlSec:AddParagraph({Title = "Statistics", Desc = "Placed: 0 | Upgraded: 0"})

FarmControlSec:AddToggle({
    Name = "🚀 Master Auto Farm", Default = false, Outline = false, Flag = "FarmToggle",
    Callback = function(v)
        if v then M.findBase() M.startFarming() OrionLib:MakeNotification({Name="Master Farm", Content="Started! Prioritizing targets...", Time=3})
        else M.stopFarming() OrionLib:MakeNotification({Name="Master Farm", Content="Stopped.", Time=3}) end
    end
})

-- ================== SISANYA (FACTORY, AUTO, CONFIGS) ==================
local FacTab = Window:MakeTab({ Name = "Factory & Events", Icon = "hammer", Glass = true, Outline = true })
local FCTSec = FacTab:AddSection({ Name = "🏭 Factory Loop", Glass = true, Outline = true })
FCTSec:AddDropdown({ Name = "Rarity", Default = M.S.FactoryRarity, Options = FR, Multi = false, Outline = true, Callback = function(v) M.S.FactoryRarity=v end })
FCTSec:AddDropdown({ Name = "Work Slot", Default = M.S.FactorySlot, Options = SL, Multi = false, Outline = true, Callback = function(v) M.S.FactorySlot=v end })
FCTSec:AddSlider({ Name = "Max Level", Min = 1, Max = 500, Default = M.S.FactoryMaxLevel, Increment = 1, ValueName = "Lv", Outline = false, Callback = function(v) M.S.FactoryMaxLevel=math.floor(v) end })
local FCSP = FCTSec:AddParagraph({Title = "Factory Status", Desc = "Idle"})
FCTSec:AddToggle({ Name = "🔁 Factory Loop Toggle", Default = false, Outline = false, Callback = function(v) if v then M.findBase() M.startFactoryLoop() else M.stopFactoryLoop() end end })

local AutoTab = Window:MakeTab({ Name = "Automation", Icon = "rocket", Glass = false, Outline = true })
local UtilSec = AutoTab:AddSection({ Name = "💰 Base Utility", Glass = false, Outline = true })
local MSP = UtilSec:AddParagraph({Title = "Money Status", Desc = "Idle"})
UtilSec:AddToggle({ Name = "💰 Auto Collect Money", Default = false, Outline = false, Callback = function(v) if v then M.findBase() M.startMoney() else M.stopMoney() end end })
local USP = UtilSec:AddParagraph({Title = "Upgrade Status", Desc = "Idle"})
UtilSec:AddToggle({ Name = "⬆️ Upgrade All Base Slots", Default = false, Outline = false, Callback = function(v) if v then M.findBase() M.startAutoUpgrade() else M.stopAutoUpgrade() end end })

local ConfTab = Window:MakeTab({ Name = "Config", Icon = "settings", Glass = false, Outline = true })
local TweakSec = ConfTab:AddSection({ Name = "Tweaks (Fixed)", Glass = false, Outline = true })

-- FIXED: Pake slider biar math / kalkulasinya 100% aman (Gak bakal kena error string / blank)
TweakSec:AddSlider({
    Name = "Farm Tween Speed",
    Min = 1, Max = 100, Default = 60, Increment = 5,
    ValueName = "Speed", Outline = false, Flag = "TweenSpeed",
    Callback = function(v) M.S.TweenSpeed = math.floor(v) end
})

TweakSec:AddDropdown({ Name = "Corridor Speed", Default = "400", Options = CSPD, Multi = false, Outline = true, Callback = function(v) M.S.CorridorSpeed=tonumber(v) or 400 end })
TweakSec:AddButton({ Name = "🏠 Find My Base", Outline = true, Callback = function() M.findBase() end })
TweakSec:AddButton({ Name = "📍 Set Home Position", Outline = true, Callback = function() M.setHomePosition() end })
local IP = ConfTab:AddParagraph({Title = "Player Info", Desc = "Loading..."})

task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            local statusColor = M.S.Farming and "'#42f563'" or "'#ff4444'"
            local currentStatus = M.S.Farming and M.Status.farm or "Idle"
            FSP:Set({
                Title = "🚀 Master Farm Status", 
                Desc = "Status: <font color="..statusColor..">" .. currentStatus .. "</font>\nBrainrots Collected: #" .. M.Status.farmCount .. "\nLBs Opened: #" .. M.Status.luckyBlockCount
            })
        end)
        
        pcall(function() FPP:Set({ Title = "📊 Statistics", Desc = "Placed: " .. M.Status.placeCount .. "\nUpgraded: " .. M.Status.upgradeCount }) end)
        pcall(function() FCSP:Set({ Title = "🏭 Factory Status", Desc = "Status: " .. (M.Status.factory or "Idle") .. "\nMaxed: #" .. M.Status.factoryCount }) end)
        
        pcall(function()
            IP:Set({ Title = "👤 Player Info", Desc = string.format("Player: %s\nBase: %s\nGod Mode: %s\nNoclip: %s", LPlayer.Name, (M.baseGUID or "Not Found"), tostring(M._isGod), tostring(M.S.NoclipEnabled)) })
        end)
        pcall(function() MSP:Set({Title = "💰 Money Status", Desc = "Status: " .. (M.S.AutoCollectMoney and "Active" or "Idle")}) end)
        pcall(function() USP:Set({Title = "⬆️ Upgrade Status", Desc = "Status: " .. (M.S.AutoUpgrade and M.Status.upgrade or "Idle") .. "\nUpgraded: #" .. M.Status.upgradeCount}) end)
    end
end)

Window:AddConfigTab({ Name = "Configs", Icon = "save" })
OrionLib:Init()